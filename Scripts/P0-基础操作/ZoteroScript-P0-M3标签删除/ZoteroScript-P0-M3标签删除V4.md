...
Project:
- BuildZotero
Title: ZoteroScript-P0-M3标签删除V4
...
(async () => {
  ...
  const userInput = prompt(
    ...
    "示例：0 或 1,3,4 或 1-1,1-2 或 Theory/, Item/ 或 因变量, 理论标签 或 完整标签: Theory/自我决定理论, Sample/1宏观 或 混合: 1,Theory/,变量定义,Sample/1宏观"
  );
  ...
  if (theoryNewOnly) {
    tagPrefixes.push("Theory/");
    selectedDescriptions.push("选项0(仅删除 Theory/*-new)");
  } else {
  ...
  for (let item of items) {
    try {
      ...
      const tagsToDelete = existingTags.filter(tag => shouldDelete(tag.tag || ''));
  ...
  return `标签删除完成！\n模式: ${theoryNewOnly ? 'Theory/*-new专用' : '普通模式'}\n前缀: [${prefixList || '无'}]\n精确: [${exactList || '无'}]\n处理条目: ${processedCount}\n删除标签: ${deletedTagsCount}\n跳过条目: ${skipCount}\n错误: ${errorCount}`;
})()
}$
