...
Project:
  - BuildZotero
Title: ZoteroScript-P1-Tag2方法标签V2
...
## Zotero-Script-P1-Tag2 方法标签 V2

```javascript
#🏷️MethodTags[color=#66dc79][trigger=]
${
(async () => {
  ...
  for (let item of items) {
    try {
      // 检查是否已有Method标签，避免重复处理
      const existingTags = item.getTags();
      const hasMethodTags = existingTags.some(tag => tag.tag && tag.tag.startsWith("Method/"));
      if (hasMethodTags) {
        skipCount++;
        continue;
      }
      ...
      if (!content) {
        // 无内容，标注便于识别
        await saveItemWithTags(item, ['Method/无内容'], 'noContent');
        continue;
      }
...
3. 第三个标签：辅助分析方法 - 研究中使用的次要分析技术，必须是具体的方法名称（3-8个字符）
   检验方法类：
...
6. 如果文献中提到多种方法，选择最核心、使用最多的具体方法

请直接返回JSON数组，格式：["方法类型", "主要分析方法", "辅助分析方法"]`;
        ...
        try {
          ...
          if (!jsonMatch) {
            await saveItemWithTags(item, ['Method/解析失败'], 'fail');
            continue;
          }

          const tags = JSON.parse(jsonMatch[0]);
          if (!tags || tags.length !== 3) {
            await saveItemWithTags(item, ['Method/格式错误'], 'fail');
            continue;
          }
          ...
          if (validTags.length !== 3) {
            await saveItemWithTags(item, ['Method/标签不完整'], 'fail');
            continue;
          }
          ...
          if (!validMethodTypes.includes(validTags[0])) {
            await saveItemWithTags(item, ['Method/方法类型无效'], 'fail');
            continue;
          }
          ...
          await saveItemWithTags(item, ['Method/JSON解析失败'], 'fail');
        }

      } catch (aiError) {
        // AI调用失败
        console.error('AI调用失败:', aiError);
        await saveItemWithTags(item, ['Method/AI调用失败'], 'fail');
      }
