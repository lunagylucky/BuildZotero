...
Project:
- BuildZotero
Title: ZoteroScript-P1-Tag2方法标签V4-Rnote
...
## ZoteroScript-P1-Tag2 方法标签 V4-Rnote（笔记优先版本）
...
1. 第一个标签:方法类型 - 必须从以下选择:
   ...
   - 方法-软件:软件工具开发
   - 方法-程序:算法程序开发
2. 第二个标签:核心分析方法 - 研究中使用最重要的分析技术,必须是具体的方法名称(3-10个字符)
   统计模型类:
...
3. 第三个标签:次要分析方法1 - 研究中使用的辅助分析技术,必须是具体的方法名称(3-10个字符)
4. 第四个标签:次要分析方法2 - 研究中使用的其他分析技术,必须是具体的方法名称(3-10个字符)
5. 第五个标签:次要分析方法3 - 研究中使用的额外分析技术,必须是具体的方法名称(3-10个字符)
   检验方法类:
...
8. 每个标签限制在10个字符以内
9. 按照重要程度排序,第1个标签是方法类型,第2-5个标签按重要性递减
10. 如果某个类别没有对应方法,返回"无"
请直接返回JSON数组,格式:["方法类型", "方法1", "方法2", "方法3", "方法4"]`;
        ...
        try {
          ...
          const validTags = tags
            ...
            .map(tag => tag.trim())
            .map(tag => tag.substring(0, 10)); // 限制每个标签最多10个字符
          ...
          // 添加标签 - 使用sMeth前缀,序号格式,不包含主要/辅助描述
