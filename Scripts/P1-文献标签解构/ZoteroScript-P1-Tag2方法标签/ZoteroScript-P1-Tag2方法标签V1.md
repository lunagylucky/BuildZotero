...
Project:
  - BuildZotero
Title: ZoteroScript-P1-Tag3方法标签V1
...
## ZoteroScript-P1-Tag2 方法标签 V1

```javascript
#🏷️MethodTagsAb[color=#66dc79][trigger=]
${
(async () => {
  ...
  for (let item of items) {
    try {
      ...
      // AI分析生成研究方法标签
      try {
        const prompt = `分析以下学术文献内容，识别其核心研究方法：
...
请直接返回JSON数组，格式：["方法类型", "具体方法1", "具体方法2"]`;
        ...
        try {
          ...
          // 添加标签 - 统一化格式
          item.addTag('Method/1' + validTags[0]);
          item.addTag('Method/' + validTags[1]);
          item.addTag('Method/' + validTags[2]);
