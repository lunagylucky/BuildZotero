...
Project:
- BuildZotero
Title: ZoteroScript-P1-Tag3样本标签V3-Rnote
...
## ZoteroScript-P1-Tag3 样本标签 V3-Rnote（笔记优先版本）

```javascript
#🏷️Rnote-SampleTag3[color=#66dc79][trigger=]
${
(async () => {
  const items = ZoteroPane.getSelectedItems();
  if (!items || items.length === 0) return "未选择任何条目";
  let successCount = 0;      // 成功添加对象标签的条目
  let skipCount = 0;         // 没有📒标签被跳过的条目
