...
Class:
Project:
Title: ZoteroScript-P1-Tag6变量标签V2-Rnote
...
## ZoteroScript-P1-Tag6 变量标签 V2-Rnote

```javascript
#🏷️Rnote-VariableTag3[color=#66dc79][trigger=]
${
(async () => {
...
示例输出3(错误格式-名称过长):
{
        ...
        try {
          ...
          for (let [key, prefix] of Object.entries(tagMapping)) {
            ...
            if (value && typeof value === 'string' && value.trim() !== '') {
              ...
              if (trimmedValue === '无') {
                if (key === 'DV') {
                  item.addTag(`${prefix}/无`);
                  tagCount++;
                }
                // 其他变量为"无"时不添加标签,直接跳过
              } else {
                // 值不为"无"时,正常添加标签
                item.addTag(`${prefix}/${trimmedValue}`);
                tagCount++;
              }
            }
          }
  ...
  return `变量标签提取完成 - 成功: ${successCount}, 跳过: ${skipCount}, 无内容: ${noContentCount}, 失败: ${failCount}, 保存失败: ${errorCount}`;
})()
}$
```
