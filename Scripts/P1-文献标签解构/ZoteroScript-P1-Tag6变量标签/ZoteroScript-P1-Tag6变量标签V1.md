...
Project:
  - BuildZotero
Title: ZoteroScript-P1-Tag6变量标签V1
...
## ZoteroScript-P1-Tag6 变量标签 V1

```javascript
#🏷️VariableTag1[color=#66dc79][trigger=]
${
(async () => {
  ...
  for (let item of items) {
    try {
      // 检查是否已有A1-A6变量标签,有则跳过
      const existingTags = item.getTags();
      const hasVariableTags = existingTags.some(tag => 
