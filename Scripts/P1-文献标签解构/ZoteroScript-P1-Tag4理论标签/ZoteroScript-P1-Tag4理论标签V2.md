...
Project:
- BuildZotero
Title: ZoteroScript-P1-Tag4理论标签V2
...
## ZoteroScript-P1-Tag4 理论标签 V2

```javascript
#Theory Tags[color=#66dc79][trigger=]
${
(async () => {
...
5. 格式：["理论名"] 或 ["无明确理论"]`;
  ...
  return `理论标签处理完成 - 成功: ${successCount}, 无理论: ${noTheoryCount}, 跳过: ${skipCount}, 失败: ${failCount}, 无摘要: ${noAbstractCount}, 无关键词: ${noKeywordCount}, 保存失败: ${errorCount}`;
})()
}$
```
