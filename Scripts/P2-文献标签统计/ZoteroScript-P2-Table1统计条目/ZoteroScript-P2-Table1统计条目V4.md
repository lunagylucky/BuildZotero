...
Project:
  - BuildZotero
Title: ZoteroScript-P2-Table1统计条目V4
...
## ZoteroScript-P2-Table1 条目表格 V4
...
(async () => {
  ...
  const modeInput = prompt(
    ...
    "5 - 结果(1列) + 样本(3列) + 方法(2列) + 条目信息(5列)\n" +
    "6 - 全列(所有22列：变量6+主题理论结果3+样本3+方法2+条目信息5)\n" +
    ...
    try {
      ...
      if (tags && tags.length > 0) {
        for (let tagObj of tags) {
          ...
          } else if (tag.startsWith(tagPrefixes.method1)) {
            // 先检查 sMeth/1（更具体的前缀）
            categorizedTags.method1.push(tag.substring(tagPrefixes.method1.length));
          } else if (tag.startsWith(tagPrefixes.methodOther)) {
            // 再检查其他所有 sMeth/
            categorizedTags.methodOther.push(tag.substring(tagPrefixes.methodOther.length));
      ...
      itemsData.push({
        ...
        method1: categorizedTags.method1.length > 0 ? categorizedTags.method1.join('; ') : "-",
        methodOther: categorizedTags.methodOther.length > 0 ? categorizedTags.methodOther.join('; ') : "-",
  ...
  switch(mode) {
    ...
    case "4": // 变量(6列) + 理论(1列) + 结果(1列) + 样本(3列)
      ...
      break;
    case "5": // 结果(1列) + 样本(3列) + 方法(2列) + 条目信息(5列)
      columns.push('Result', 'Sample1', 'Sample2', 'Sample3', 'Method1', 'MethodOther', 'V1-变量定义', 'V2-测量方法', 'V3-研究贡献', 'V4-研究展望', 'V5-文献来源');
      break;
    case "6": // 全列(完整版 - 包含所有22列，变量6列不含OV)
      columns.push('A1-DV', 'A2-IV', 'A3-MO', 'A4-ME', 'A5-INV', 'A6-CV', 'Item', 'Theory', 'Result', 'Sample1', 'Sample2', 'Sample3', 'Method1', 'MethodOther', 'V1-变量定义', 'V2-测量方法', 'V3-研究贡献', 'V4-研究展望', 'V5-文献来源');
      break;
  ...
  itemsData.forEach((data, index) => {
    ...
    switch(mode) {
      ...
      case "4":
        ...
        break;
      case "5":
        row += ` | ${data.result} | ${data.sample1} | ${data.sample2} | ${data.sample3} | ${data.method1} | ${data.methodOther} | ${data.v1def} | ${data.v2mea} | ${data.v3con} | ${data.v4fut} | ${data.v5sor}`;
        break;
      case "6": // 全列(完整版 - 所有22列，变量6列不含OV)
        row += ` | ${data.a1dv} | ${data.a2iv} | ${data.a3mo} | ${data.a4me} | ${data.a5inv} | ${data.a6cv} | ${data.item} | ${data.theory} | ${data.result} | ${data.sample1} | ${data.sample2} | ${data.sample3} | ${data.method1} | ${data.methodOther} | ${data.v1def} | ${data.v2mea} | ${data.v3con} | ${data.v4fut} | ${data.v5sor}`;
        break;
      case "7":
 ...
 * 
 * 【V5-2 版更新说明】
 * - 选项 5（14 列）：结果 + 样本 3 列 + 方法 2 列 + 条目信息 5 列
 * · 基础信息：文献、年份、期刊（3 列）
 * · 结果：Result（1 列）
 * · 样本标签：Sample1, Sample2, Sample3（3 列）
 * · 方法标签：Method1(sMeth/1), MethodOther(其他所有 sMeth/)（2 列）
 * · 条目信息：V1- 变量定义, V2- 测量方法, V3- 研究贡献, V4- 研究展望, V5- 文献来源（5 列）
 * 
 * - 选项 6（22 列）：全列模式（变量 6 列，不含 OV）
 * · 基础信息：文献、年份、期刊（3 列）
 * · 变量标签：A1-DV, A2-IV, A3-MO, A4-ME, A5-INV, A6-CV（6 列，不含 OV）
 * · 主题理论结果：Item, Theory, Result（3 列）
 * · 样本标签：Sample1, Sample2, Sample3（3 列）
 * · 方法标签：Method1(sMeth/1), MethodOther(其他所有 sMeth/)（2 列）
 * · 条目信息：V1- 变量定义, V2- 测量方法, V3- 研究贡献, V4- 研究展望, V5- 文献来源（5 列）  
 */
