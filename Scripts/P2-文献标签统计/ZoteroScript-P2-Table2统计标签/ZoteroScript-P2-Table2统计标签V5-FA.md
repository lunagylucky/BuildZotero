...
Project:
  - BuildZotero
Title: ZoteroScript-P2-Table2统计标签V5-FA
...
## ZoteroScript-P2-Table2 统计标签 V5-FA

```javascript

#🏷️FA-Ttags[color=#2196F3][trigger=]  
${  
(async () => {  
  ...
  const tagMapping = {  
    ...
    "7-4": "V4-fut/",
    "7-5": "V5-sor/",
    "7": ["V1-def/", "V2-mea/", "V3-con/", "V4-fut/", "V5-sor/"]
  };
  
  // 用户输入选择 - 第一步：选择前缀  
  const userInput = prompt(  
    "【第 1 步】请输入要统计的标签类型（多个用逗号分隔）：\n\n" +  
    "【数字选项】\n" +  
    "1 - 变量标签（全部 6 个）\n" +  
    ...
    "7 - 条目细节标签（研究要素，全部 5 个）\n" +  
    "  7-1 变量定义 (V1-def)  7-2 测量方法 (V2-mea)\n" +  
    "  7-3 研究贡献 (V3-con)  7-4 研究展望 (V4-fut)\n" +  
    "  7-5 文献来源 (V5-sor)\n\n" +  
    "【自定义前缀】\n" +  
    " 也可直接输入前缀，如：sMeth/, Theory/, Item/, Result/, Sample/1\n\n" +  
    " 示例：1,3,4 或 Theory/, Item/ 或 混合使用: 1,Theory/"  
  );
  ...
  summary += `- 筛选条件：\n`;  
  summary += `  - 前缀：${uniquePrefixes.join(', ')}\n`;  
  if (keywords.length > 0) {  
    summary += `  - 关键词：${keywords.join(' + ')} (同时包含)\n`;  
  } else {  
    summary += `  - 关键词：无（显示所有标签）\n`;  
  }  
  summary += `- 分组不同标签数：\n`;
  ...
  for (let prefix of uniquePrefixes) {  
    ...
    totalMatchedTags += distinctCount;  
    summary += `  - ${prefix} ${distinctCount}\n`;  
  }  
  summary += `- 匹配标签总数：${totalMatchedTags}\n`;  
  summary += `\n`;  
  result += summary;
  ...
  if (totalMatchedTags === 0) {  
    result = '### 未找到匹配的标签\n\n';  
    result += `- 筛选条件：\n`;  
    result += `  - 前缀：${uniquePrefixes.join(', ')}\n`;  
    if (keywords.length > 0) {  
      result += `  - 关键词：${keywords.join(' + ')} (必须同时包含)\n`;  
      result += `\n提示：标签必须同时包含所有关键词才会显示，请检查关键词拼写或尝试使用单个关键词`;  
    }  
  }
 ...
 * 新增功能：
 * - 两步输入：先选择前缀，再输入关键词
 * - 关键词筛选：仅显示同时包含所有指定关键词的标签（且关系/AND 逻辑）
 * - 灵活使用：关键词留空时等同于原版功能
 * 
 * 与版本 5 的区别：
 * - 版本 5：包含任意一个关键词即可（或关系/OR 逻辑）
 * - 版本 6：必须同时包含所有关键词（且关系/AND 逻辑）  
 */
