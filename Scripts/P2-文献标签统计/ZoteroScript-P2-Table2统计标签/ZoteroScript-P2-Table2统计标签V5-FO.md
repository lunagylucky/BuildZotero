...
Project:
  - BuildZotero
Title: ZoteroScript-P2-Table2统计标签V5-FO
...
## ZoteroScript-P2-Table2 统计标签 V5-FO

```Java

#🏷️FO-Ttags[color=#2196F3][trigger=]  
${  
(async () => {  
  ...
  if (keywords.length > 0) {  
    summary += `  - 关键词：${keywords.join(', ')} (任意匹配)\n`;  
  } else {  
 ...
 * 新增功能：
 * - 两步输入：先选择前缀，再输入关键词
 * - 关键词筛选：仅显示包含指定关键词的标签（支持多个关键词，任意匹配）
 * - 灵活使用：关键词留空时等同于原版功能  
 */
